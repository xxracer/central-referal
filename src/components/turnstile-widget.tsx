...
export function TurnstileWidget({ siteKey, onVerify }: TurnstileWidgetProps) {
    ...
    useEffect(() => {
        if (isLoaded && containerRef.current && (window as any).turnstile) {
            ...
            // Force testing key in dev mode, otherwise use prop -> env -> fallback
            const effectiveSiteKey = isDev ? testingSiteKey : (siteKey || process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY || "0x4AAAAAACaQtV2HCSGPHGiQ");
